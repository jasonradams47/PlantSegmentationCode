####This file contains the method comparison on the reduced unbalanced training and testing sets####
####(results found in table 1). Note that for the methods that contain some kind of random      ####
...
#create training and testing sets from total data set#
#set.seed(5118)
#train.index<-sample(length(Y),round(.05*length(Y)),replace=F)
#train.dat<-cbind(X[train.index,],Y[train.index])

#rm<-cbind(X[-train.index,],Y[-train.index])
#test.index<-sample(dim(rm)[1],10000,replace=F)
#test.dat<-rm[test.index,]

tr.nb<-readRDS("Data/UnbalMethodCompTr.RDS")
ts.nb<-readRDS("Data/UnbalMethodCompTs.RDS")
...
###Use t1=0.2,t2=0.135 to obtain error rates###
ptr.thresh<-thresh.on.matrix(tr.rgb[,1:3],.2,0.135,c(-1,2,-1))
...
pts.thresh<-thresh.on.matrix(ts.rgb[,1:3],0.2,0.135,c(-1,2,-1))
...
###Use results of parameter serach (t=0) to obtain error rates###
ptr.thresh<-bin.thresh(tr.rgb[,1:3],0)
table(pred=ptr.thresh,truth=tr.rgb[,4])                    #confusion matrix
prop.table(table(pred=ptr.thresh,truth=tr.rgb[,4]))        #c. matrix proportions
length(which(ptr.thresh!=tr.rgb[,4]))/dim(tr.rgb)[1] 

pts.thresh<-bin.thresh(ts.rgb[,1:3],0)
table(pred=pts.thresh,truth=ts.rgb[,4])                    #confusion matrix
prop.table(table(pred=pts.thresh,truth=ts.rgb[,4]))        #c. matrix proportions
